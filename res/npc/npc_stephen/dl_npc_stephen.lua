...
loadDialogue = function(DL) 
	...
	if (not DL:isConditionFulfilled("npc_stephen", "talked")) then 
		...
		DL:createNPCNode(1, 3, "DL_NPC_ANewFace") -- And again, a new face in the city. As if we hadn't already troublemakers here. 
	...
	DL:createNPCNode(10, -2, "DL_NPC_Rules3") -- And, last but not least, we don't really like seeing other spells than those which originate from the high divine art. You can hang for practicing Necromancy.
	DL:addNode()
	...
	if (not DL:isConditionFulfilled("npc_stephen", "who_are_you")) then 
		...
		DL:addChoice(8, "DL_Choice_NotUnderstood") -- ... (Grin disparagingly)
		DL:addNode()
